...
model.add(Convolution2D(filter_num, kernel_size=(3, 3), strides=(1,1), padding='same'))
model.add(advanced_activations.LeakyReLU(alpha=0.05))
model.add(BatchNormalization())
...
# Conv Block 4
model.add(Convolution2D(filter_num*8, kernel_size=(3, 3), strides=(1,1), padding='same'))
model.add(advanced_activations.LeakyReLU(alpha=0.05))
model.add(BatchNormalization())
model.add(Convolution2D(filter_num*8, kernel_size=(3, 3), strides=(1,1), padding='same'))
model.add(advanced_activations.LeakyReLU(alpha=0.05))
...
if (data_augmentation):
    ...
    checkpointer = ModelCheckpoint(filepath='model_17.h5', monitor='val_acc', verbose=0, save_best_only=True, mode='auto')
