...
from keras.layers import BatchNormalization, Convolution2D, MaxPooling2D, Flatten, advanced_activations, ZeroPadding2D
...
# Conv Block 1 // 64
model.add(Convolution2D(filter_num, kernel_size=(3, 3), strides=(1,1), padding='same', kernel_initializer='he_uniform', input_shape=x_data.shape[1:]))
model.add(advanced_activations.LeakyReLU(alpha=0.05))
model.add(BatchNormalization())

model.add(Convolution2D(filter_num, kernel_size=(3, 3), strides=(1,1), padding='same', kernel_initializer='he_uniform'))
model.add(advanced_activations.LeakyReLU(alpha=0.05))
model.add(BatchNormalization())
...
# Conv Block 2 // 128
model.add(Convolution2D(filter_num*2, kernel_size=(3, 3), strides=(1,1), padding='same',kernel_initializer='he_uniform'))
model.add(advanced_activations.LeakyReLU(alpha=0.05))
model.add(BatchNormalization())
...
# Conv Block 3 // 256
model.add(Convolution2D(filter_num*4, kernel_size=(3, 3), strides=(1,1), padding='same', kernel_initializer='he_uniform'))
model.add(advanced_activations.LeakyReLU(alpha=0.05))
model.add(BatchNormalization())
...
# Conv Block 4 // 512
model.add(Convolution2D(filter_num*8, kernel_size=(3, 3), strides=(1,1), padding='same', kernel_initializer='he_uniform'))
model.add(advanced_activations.LeakyReLU(alpha=0.05))
model.add(BatchNormalization())
...
if (data_augmentation):
    ...
    checkpointer = ModelCheckpoint(filepath='model_22.h5', monitor='val_acc', verbose=0, save_best_only=True, mode='auto')
